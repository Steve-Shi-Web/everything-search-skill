...
def check_port_listening():
    ...
    try:
        ...
        if result == 0:
            ...
            print("   → Solution: Enable HTTP Server in Everything (Ctrl+P → HTTP Server)")
            return False
    except Exception as e:
        print(f"   ? Error: {e}")
        return False
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=5) as response:
            ...
            if response.status == 200:
                print(f"   ✓ HTTP server responding (Status: 200, Time: {elapsed:.2f}s)")
                return True
            else:
                ...
                with open(config_path, 'r', encoding='utf-8') as f:
                    ...
                    if "http_server" in content.lower():
                        ...
                        if "enabled=1" in content:
                            ...
                            print("   → Note: Config file changes don't auto-enable HTTP server")
                            print("   → Must enable via GUI: Ctrl+P → HTTP Server → Check 'Enable'")
...
def main():
    ...
    if passed == total:
        ...
        print("  - python examples/search_photos.py \"史周平\"")
