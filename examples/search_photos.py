...
def main():
    if len(sys.argv) < 2:
        ...
        print("Example: python search_photos.py 史周平")
        sys.exit(1)
    ...
    try:
        ...
        if results.total == 0:
            ...
            # Group by type
            jpg_photos = [item for item in results.items if item.name.lower().endswith('.jpg')]
            png_photos = [item for item in results.items if item.name.lower().endswith('.png')]
